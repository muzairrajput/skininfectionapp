...
until docker exec mysql_master sh -c 'export MYSQL_PWD={PWD}; mysql -u root -e ";"'
do
...
until docker-compose exec mysql_slave sh -c 'export MYSQL_PWD={PWD}; mysql -u root -e ";"'
do
...
docker exec mysql_slave sh -c "export MYSQL_PWD={PWD}; mysql -u root -e 'SHOW SLAVE STATUS \G'"
